...
const OrderDetailsPage = ({ onClose, order }) => {
  ...
  //   console.log("pic u ",photos);
  //   setPhotoSlider(photos);
  //   console.log("slide ",photoSlider);
  ...
  const handleProductClick = (photos) => {
    ...
    if (Array.isArray(photos)) {
      ...
      // handle case if it's object (e.g., {url: "..."} or something)
      images = Object.values(photos);
    }

    setPhotoSlider(images);
    setSelectedPhoto(images[0] || null);
  };
          ...
          <tbody>
            {order.products.map((item, index) => (
              ...
              >
                ...
                <td className="border p-2 md:p-3 text-center">{item.userSize}</td>
                {/* <td className="border p-2 md:p-3 text-center">{item.product.number}</td> */}
                <td className="border p-2 md:p-3 text-center">{item.userColor}</td>
                ...
                <td className="border p-2 md:p-3 text-left w-[250px]">{item.product.description}</td>
                <td className="border p-2 md:p-3 text-center">{console.log(item.item_img)}
                  <button
